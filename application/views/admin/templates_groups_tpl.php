    ...
    <div id="page_body">
        <div class="tabs">
			<ul>
                ...
                <?php if ($components_privileges->templates_groups->edit && (isset($formdata['edit']) || @$group->id)) { ?>
				<li><a href="#edit_group"><?php echo $lang->line('ACT_EDIT'); ?></a></li>
                <?php } ?>
				<?php if ($components_privileges->templates_groups->add) { ?>
			...
			<div id="groups">
				...
				<table class="fields_line" cellspacing="0" cellpadding="0">
					<tr>
                        ...
                        <?php if ($components_privileges->templates_groups->delete) { ?>
						<th><?php echo $lang->line('ACT_DELETE'); ?></th>
                        <?php } ?>
					</tr>
                    <?php $row_class = 'row_a'; foreach ($templates_groups->result() as $row) { ?>
					<tr class="<?php echo $row_class; ?>">
						<td><?php echo ++$groups_position_number; ?></td>
						<td class="row_title"><?php if ($components_privileges->templates_groups->edit) { ?><a href="/admin/templates_groups/edit_group/<?php echo $row->id; ?>#edit_group" class="edit_row_link" title="<?php echo $lang->line('ACT_EDIT'); ?>"><?php } else { ?><a><?php } ?><?php echo $row->title; ?></a></td>
						<td><?php echo $row->comments; ?></td>
                        <?php if ($components_privileges->templates_groups->delete) { ?>
						<td><a href="/admin/templates_groups/delete_group/<?php echo $row->id; ?>" class="delete_row_link" title="<?php echo $lang->line('ACT_DELETE'); ?>"></a></td>
                        <?php } ?>
					</tr>
            ...
            <?php if ($components_privileges->templates_groups->edit && (isset($formdata['edit']) || @$group->id)) { ?>
			<div id="edit_group">
	            <form method="post" action="/admin/templates_groups/edit_group/<?php echo $group->id; ?>#edit_group">
                	<table class="fields_line" cellspacing="0" cellpadding="0">
                    	<tr>
			...
			<?php if ($components_privileges->templates_groups->add) { ?>
			<div id="add_group">
	            <form method="post" action="/admin/templates_groups/add_group">
                	<table class="fields_line" cellspacing="0" cellpadding="0">
                    	<tr>
                        	<th><?php echo $lang->line('CONTENT_FORM_PARAMETER'); ?></th>
                            <th><?php echo $lang->line('CONTENT_FORM_VALUE'); ?></th>
